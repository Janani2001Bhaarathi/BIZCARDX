...
img = Image.open(r"C:\Users\JANANI BHAARATHI\OneDrive\Desktop\Project 3\Bizcardx.png")
    ...
    if image is not None:
        ...
        if selected == "Preview":
            ...
            if Upload:
                ...
                mydb.commit()
                mycursor.execute("SELECT email FROM BUSINESS_CARD")
                # Fetch all results
                emails = mycursor.fetchall()
                # Extract emails from the fetched results and remove duplicates using a set
                email_list = list(set([email[0] for email in emails]))
                if modified_m in email_list:
                    st.warning('REPEATED ENTRY', icon="⚠️")
                else:    
...
if selected == "About":
    ...
    if selected == "Personal":
        ...
        col2.image(Image.open(r'C:\Users\JANANI BHAARATHI\OneDrive\Desktop\Project 3\JANANI BHAARATHI K M.jpeg'), width=400)
        with col1:
